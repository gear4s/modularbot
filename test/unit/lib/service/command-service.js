...
describe(testName(module), () => {
  ...
  it("should fail to register command without command type", async function() {
    ...
    try {
      ...
      expect(e).to.have.property("message").which.equals("Command must be an object of the Chainable Command class");
    }
  });
  ...
  it("should fail to register a duplicate command", async function() {
    ...
    try {
      ...
      expect(e).to.have.property("message").which.equals("Command `test1` already exists");
    }
  });
  ...
  it("should throw an error relating to prefix containing strings", async function() {
    ...
    try {
      ...
      expect(e).to.have.property("message").which.equals("Iterable `defaultPrefix` or list returned from `prefix` must contain only strings")
    }
  });
