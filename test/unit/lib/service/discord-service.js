...
describe(testName(module), () => {
  ...
  it("should create an instance of the discord service", function() {
    ...
    expect(this.mockDiscordClient.on).to.have.been.calledWithExactly("ready", sinon.match.func);
  });
  ...
  it("should handle ready states", function() {
    ...
    expect(this.mockLogger.debug).to.have.been.calledWithExactly(`Logged in as ${this.mockDiscordClient.user.tag}!`);
  });

  it("should return a new Discord instance", function() {
    const svc = new DiscordService(this.mockLogger, this.mockConfig, this.mockLibraries);
