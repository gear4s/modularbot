...
export default class TestCommandService {
  ...
  async init() {
    const twitchResponse = await this.#axios.post(
      `https://id.twitch.tv/oauth2/token?client_id=${
        this.#config.twitch.publicKey
      }&client_secret=${
        this.#config.twitch.secretKey
    ...
    this.#services.command.register(
      new this.#utils.command.ChainableCommand()
        ...
        .withName("watch")
        .withArgs([String, this.#discord.TextChannel])
    );
  }
  ...
  checkIfStreamerIsLive = async (ctx, streamerName) => {
    ...
    } = await this.#helix(`streams?user_login=${streamerName}`);
    const {
      data: {
        data: [channelData],
      },
    } = await this.#helix(`users?login=${streamerName}`);
  ...
  watchForStreamerToBeLive = async (
    ctx: ContextUtil,
    streamerName: string,
